 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').factory('ngTableEventsChannel',
		 ...
		 * 
		 * Supported events:
		 *  * afterCreated - raised when a new instance of `NgTableParams` has
		 * finished being constructed * afterReloadData - raised when the
		 * `reload` event has finished loading new data * datasetChanged -
		 * raised when `settings` receives a new data array * pagesChanged -
		 * raised when a new pages array has been generated
		 */
		function ngTableEventsChannel($rootScope) {
			...
			function addChangeEvent(eventName, target) {
				var fnName = eventName.charAt(0).toUpperCase()
			...
			function createEventSubscriptionFn(eventName) {
				...
				return function subscription(handler/*
					...
					// shorthand for subscriber to only receive events from a
					...
					return scope.$on('ngTable:' + eventName, function(event,
									params/* , ...args */) {
								// don't send events published by the internal
								// NgTableParams created by ngTableController
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').provider('ngTableFilterConfig',
		...
		function ngTableFilterConfigProvider() {
			...
			function setConfig(customConfig) {
				...
				mergeConfig.aliasUrls = angular.extend({}, config.aliasUrls,
			...
			function ngTableFilterConfig() {
				...
				function getUrlForAlias(aliasName/* , filterKey */) {
					return config.aliasUrls[aliasName] || config.defaultBaseUrl
							+ aliasName + config.defaultExt;
				}
			}
		}
	})();
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').provider('ngTableDefaultGetData',
		 ...
		 * @description Allows for the configuration of the
		 *              ngTableDefaultGetData service.
		 * 
		 * Set filterFilterName to the name of a angular filter that knows how
		 * to apply the values returned by `NgTableParams.filter()` to restrict
		 * an array of data.
		 * 
		 * Set sortingFilterName to the name of a angular filter that knows how
		 * to apply the values returned by `NgTableParams.orderBy()` to sort an
		 * array of data.
		 * 
		 * Out of the box the `ngTableDefaultGetData` service will be configured
		 * to use the angular `filter` and `orderBy` filters respectively
		 */
		function ngTableDefaultGetDataProvider() {
			 ...
			 * @description A default implementation of the getData function
			 *              that will apply the `filter`, `orderBy` and paging
			 *              values from the `NgTableParams` instance supplied to
			 *              the data array supplied.
			 * 
			 * The outcome will be to return the resulting array and to assign
			 * the total item count after filtering to the `total` of the
			 * `NgTableParams` instance supplied
			 */
			function ngTableDefaultGetData($filter) {
				...
				function getFilterFn(params) {
					...
					if (angular.isFunction(filterOptions.filterFn)) {
						...
						return $filter(filterOptions.filterFilterName
								|| provider.filterFilterName);
					}
				}
				...
				function applyFilter(data, params) {
					...
					return filterFn.call(params, data, parsedFilter, params
									.settings().filterOptions.filterComparator);
				}

				function applyPaging(data, params) {
					var pagedData = data.slice((params.page() - 1)
									* params.count(), params.page()
									* params.count());
					params.total(data.length); // set total for recalc
												// pagination
					return pagedData;
				}
				...
				function getData(data, params) {
					...
					var options = angular.extend({}, defaultDataOptions, params
									.settings().dataOptions);

					var fData = options.applyFilter
							? applyFilter(data, params)
							: data;
					var orderedData = options.applySort ? applySort(fData,
							params) : fData;
					return options.applyPaging ? applyPaging(orderedData,
							params) : orderedData;
				}
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').factory('ngTableGetDataBcShim',
		...
		function ngTableGetDataBcShim($q) {
			...
			function createWrapper(getDataFn) {
				return function getDataShim(/* args */) {
					var $defer = $q.defer();
					var pData = getDataFn.apply(this, [$defer]
									.concat(Array.prototype.slice
											.call(arguments)));
					if (!pData) {
						// If getData resolved the $defer, and didn't promise us
						// data,
						// create a promise from the $defer. We need to return a
						// promise.
						pData = $defer.promise;
					}
					return pData;
 ...
 */
	...
	(function() {
		 ...
		 * @name ngTableColumn
		 * @module ngTable
		 * @description Service to construct a $column definition used by
		 *              {@link ngTable ngTable} directive
		 */
		angular.module('ngTable').factory('ngTableColumn', [function() {
			 ...
			 * 
			 * @param {Object}
			 *            column an existing $column or simple column data
			 *            object
			 * @param {Scope}
			 *            defaultScope the $scope to supply to the $column
			 *            getter methods when not supplied by caller
			 * @param {Array}
			 *            columns a reference to the columns array to make
			 *            available on the context supplied to the $column
			 *            getter methods
			 * @returns {Object} a $column object
			 */
			function buildColumn(column, defaultScope, columns) {
				// note: we're not modifying the original column object. This
				// helps to avoid unintended side affects
		...
		// wrap raw field values with "getter" functions
			// - this is to ensure consistency with how ngTable.compile builds
			// columns
			// - note that the original column object is being "proxied"; this
			// is important
			// as it ensure that any changes to the original object will be
		...
		(function	(prop1) {
						// satisfy the arguments expected by the function
						// returned by parsedAttribute in the ngTable directive
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').factory(
				'NgTableParams',
				[
						'$q',
						'$log',
						...
						'ngTableEventsChannel',
						function($q, $log, $filter, ngTableDefaults,
								ngTableGetDataBcShim, ngTableDefaultGetData,
								ngTableEventsChannel) {
							var isNumber = function(n) {
								return !isNaN(parseFloat(n)) && isFinite(n);
							};
							var NgTableParams = function(baseParameters,
									baseSettings) {

								// the ngTableController "needs" to create a
								// dummy/null instance and it's important to
								// know whether an instance
								// is one of these
								...
								var self = this, prevParamsMemento, errParamsMemento, isCommittedDataset = false, initialEvents = [], log = function() {
									...
									filterComparator : undefined, // look for
																	// a
																	// substring
																	// match in
																	// case
																	...
																	// way
									filterDelay : 500,
									filterDelayThreshold : 10000, // size of
																	// dataset
																	// array
																	// that will
																	// trigger
																	...
																	// being
																	// applied
									filterFilterName : undefined, // when
																	// defined
																	// overrides
																	// ngTableDefaultGetDataProvider.filterFilterName
									filterFn : undefined, // when defined
															// overrides the
															// filter function
															// that
															// ngTableDefaultGetData
															// uses
									filterLayout : 'stack' // alternative:
															// 'horizontal'
								}, defaultGroupOptions = {
									defaultSort : 'asc', // set to 'asc' or
															// 'desc' to apply
															// sorting to groups
									isExpanded : true
								 ...
								 * @description Set new parameters or get
								 *              current parameters
								 * 
								 * @param {string}
								 *            newParameters New parameters
								 * @param {string}
								 *            parseParamsFromUrl Flag if parse
								 *            parameters like in url
								 * @returns {Object} Current parameters or
								 *          `this`
								 */
									...
									if (angular.isDefined(newParameters)) {
										for (var key in newParameters) {
											...
											if (parseParamsFromUrl
												...
												params[lastKey] = angular
														.extend(params[lastKey]
																		|| {},
																value[lastKey]);
											} else {
												if (key === 'group') {
								...
								function parseGroup(group) {
									var defaultSort = settings.groupOptions
											&& settings.groupOptions.defaultSort;
								...
								this.settings = function(newSettings) {
									if (angular.isDefined(newSettings)) {
										...
										// todo: don't modify newSettings
										// object: this introduces unexpected
										// side effects;
										// instead take a copy of newSettings

										if (newSettings.filterOptions) {
											newSettings.filterOptions = angular
													.extend(
															{},
										...
										if (newSettings.groupOptions) {
											newSettings.groupOptions = angular
													.extend(
															{},
										...
										if (angular
												.isArray(newSettings.dataset)) {
											// auto-set the total from passed in
											// dataset
											newSettings.total = newSettings.dataset.length;
										}

										// todo: remove the backwards
										// compatibility shim and the following
										// two if blocks
										if (newSettings.getData
												&& newSettings.getData.length > 1) {
											// support the old getData($defer,
											// params) api
											newSettings.getDataFnAdaptor = ngTableGetDataBcShim;
										}
										if (newSettings.getGroups
												&& newSettings.getGroups.length > 2) {
											// support the old getGroups($defer,
											// params) api
										...
										// note: using != as want null and
										// undefined to be treated the same
										var hasDatasetChanged = newSettings
												.hasOwnProperty('dataset')
												&& (newSettings.dataset != originalDataset);
										if (hasDatasetChanged) {
											if (isCommittedDataset) {
												this.page(1); // reset page as
																// a new dataset
																// has been
																// supplied
											}
								 ...
								 * @description If parameter page not set return
								 *              current page else set current
								 *              page
								 * 
								 * @param {string}
								 *            page Page number
								 * @returns {Object|Number} Current page or
								 *          `this`
								 */
								this.page = function(page) {
									return angular.isDefined(page) ? this
											.parameters({
														'page' : page
													}) : params.page;
								};
								 ...
								 * @description If parameter total not set
								 *              return current quantity else set
								 *              quantity
								 * 
								 * @param {string}
								 *            total Total quantity of items
								 * @returns {Object|Number} Current page or
								 *          `this`
								 */
								this.total = function(total) {
									return angular.isDefined(total) ? this
											.settings({
														'total' : total
													}) : settings.total;
								};
								 ...
								 * @description If parameter count not set
								 *              return current count per page
								 *              else set count per page
								 * 
								 * @param {string}
								 *            count Count per number
								 * @returns {Object|Number} Count per page or
								 *          `this`
								 */
								this.count = function(count) {
									// reset to first page because can be blank
									// page
									return angular.isDefined(count) ? this
											.parameters({
														'count' : count,
														'page' : 1
													}) : params.count;
								 ...
								 * @description If 'filter' parameter not set
								 *              return current filter else set
								 *              current filter
								 * 
								 * Note: when assigning a new filter,
								 * {@link NgTableParams#page page} will be set
								 * to 1
								 * 
								 * @param {Object|Boolean}
								 *            filter 'object': new filter to
								 *            assign or 'true': to return the
								 *            current filter minus any
								 *            insignificant values (null,
								 *            undefined and empty string); or
								 *            'falsey': to return the current
								 *            filter "as is"
								...
								this.group = function(group, sortDirection) {
									...
									} else if (angular.isDefined(group)
											&& angular.isDefined(sortDirection)) {
										var groupArray = {};
								 ...
								 * @description If 'sorting' parameter is not
								 *              set, return current sorting.
								 *              Otherwise set current sorting.
								 * 
								 * @param {string}
								 *            sorting New sorting
								...
								this.sorting = function(sorting) {
									...
									return angular.isDefined(sorting) ? this
											.parameters({
														'sorting' : sorting
													}) : params.sorting;
								};
								 ...
								 *            field Field name
								 * @param {string}
								 *            direction Optional direction of
								 *            sorting ('asc' or 'desc')
								 * @returns {Array} Return true if field sorted
								 *          by direction
								 */
								this.isSortBy = function(field, direction) {
									if (direction !== undefined) {
										return angular
												.isDefined(params.sorting[field])
								 ...
								 * @description Return object of sorting
								 *              parameters for angular filter
								 * 
								 * @returns {Array} Array like: [ '-name',
								 *          '+age' ]
								 */
								...
								function convertSortToOrderBy(sorting) {
									...
									for (var column in sorting) {
										result.push((sorting[column] === "asc"
												? "+"
												: "-")
												+ column);
									}
								 ...
								 * 
								 * When no arguments supplied, the current
								 * parameter state of this `NgTableParams`
								 * instance will be used
								 * 
								 * @param {boolean}
								 *            currentPage which page must be
								 *            active
								 * @param {boolean}
								 *            totalItems Total quantity of items
								 * @param {boolean}
								 *            pageSize Quantity of items on page
								 * @param {number}
								 *            maxBlocks Quantity of blocks for
								 *            pagination
								 * @returns {Array} Array of pages
								 */
								this.generatePagesArray = function(currentPage,
										totalItems, pageSize, maxBlocks) {
									if (!arguments.length) {
									...
									maxBlocks = maxBlocks && maxBlocks < 6
											? 6
											: maxBlocks;
									...
									if (numPages > 1) {
										...
										maxPivotPages = Math
												.round((settings.paginationMaxBlocks - settings.paginationMinBlocks)
														/ 2);
								 ...
								 * @description Return true when a change to
								 *              this `NgTableParams` instance
								 *              should require the reload method
								 *              to be run so as to ensure the
								 *              data presented to the user
								 *              reflects the `NgTableParams`
								 * 
								 * Note that this method will return false when
								 * the reload method has run but fails. In this
								 * case `hasErrorState` will return true.
								 */
								this.isDataReloadRequired = function() {
									// note: using != as want to treat null and
									// undefined the same
									return !isCommittedDataset
											|| !angular.equals(
													createComparableParams(),
													prevParamsMemento)
								 ...
								 *              NgTableParams#filter has
								 *              significant filter value(s) (any
								 *              value except null, undefined, or
								 *              empty string)
								 * @returns {Boolean} true when
								 *          NgTableParams#filter has at least
								 *          one significant field value
								 */
								 ...
								 * @description Determines if at least one group
								 *              has been set
								 * @returns {Boolean}
								 */
								this.hasGroup = function(group, sortDirection) {
									if (group == null) {
										return angular.isFunction(params.group)
												|| Object.keys(params.group).length > 0
									}
									...
									if (angular.isFunction(group)) {
										...
										if (sortDirection == null) {
											return Object.keys(params.group)
													.indexOf(group) !== -1;
										} else {
								 ...
								 * @description Return true when a change to
								 *              `NgTableParams.filters`require
								 *              the reload method to be run so
								 *              as to ensure the data presented
								 *              to the user reflects these
								 *              filters
								 */
								this.hasFilterChanges = function() {
									var previousFilter = (prevParamsMemento && prevParamsMemento.params.filter);
									return !angular.equals((params.filter),
											previousFilter)
											|| hasGlobalSearchFieldChanges();
								};
								...
								function hasGlobalSearchFieldChanges() {
									...
									var previousVal = (prevParamsMemento
											&& prevParamsMemento.params.filter && prevParamsMemento.params.filter.$);
									return !angular.equals(currentVal,
											previousVal);
								}
								 ...
								 * 
								 * @param {boolean}
								 *            asString flag indicates return
								 *            array of string or object
								 * @returns {Array} If asString = true will be
								 *          return array of url string
								 *          parameters else key-value object
								 */
								...
								this.reload = function() {
									...
									prevParamsMemento = angular
											.copy(createComparableParams());
									...
									return pData.then(function(data) {
										...
										self.data = data;
										// note: I think it makes sense to
										// publish this event even when data ===
										// oldData
										// subscribers can always set a filter
										// to only receive the event when data
										// !== oldData
										ngTableEventsChannel
												.publishAfterReloadData(self,
														data, oldData);
										self.reloadPages();

										// todo: remove after acceptable
										// depreciation period
										if (settings.$scope) {
											settings.$scope
								 ...
								 * @description Return true when an attempt to
								 *              `reload` the current `parameter`
								 *              values have resulted in a
								 *              failure
								 * 
								 * This method will continue to return true
								 * until the reload is successfully called or
								 * when the `parameter` values have changed
								 */
								this.hasErrorState = function() {
									return !!(errParamsMemento && angular
											.equals(errParamsMemento,
													createComparableParams()));
								};

								function optimizeFilterDelay() {
									// don't debounce by default filter input
									// when working with small synchronous
									// datasets
								...
								this.reloadPages = (function() {
									...
									return function() {
										...
										var newPages = self.generatePagesArray(
												self.page(), self.total(), self
														.count());
										if (!angular.equals(oldPages, newPages)) {
											currentPages = newPages;
											ngTableEventsChannel
													.publishPagesChanged(this,
															newPages, oldPages);
										}
									}
								})();
								...
								function runGetData() {
									var getDataFn = settings
											.getDataFnAdaptor(settings.getData);
									return $q.when(getDataFn.call(settings,
											self));
								}

								function runGetGroups() {
									var getGroupsFn = settings
											.getGroupsFnAdaptor(settings.getGroups);
									return $q.when(getGroupsFn.call(settings,
											self));
								}
								...
								function runInterceptorPipeline(fetchFn) {
									...
									return interceptors.reduce(function(result,
											interceptor) {
										var thenFn = (interceptor.response && interceptor.response
												.bind(interceptor))
												|| $q.when;
										var rejectFn = (interceptor.responseError && interceptor.responseError
												.bind(interceptor))
												|| $q.reject;
								...
								function getDefaultSettingFns() {
									 ...
									 * @description Returns the data to display
									 *              in the table
									 * 
									 * Called by `NgTableParams` whenever it
									 * considers new data is to be loaded
									 * 
									 * @param {Object}
									 *            params the `NgTableParams`
									 *            requesting data
									 */
									function getData(params) {
										return ngTableDefaultGetData(params
														.settings().dataset,
												params);
									}
									 ...
									 * @description Return groups of data to
									 *              display in the table
									 * 
									 * Called by `NgTableParams` whenever it
									 * considers new data is to be loaded and
									 * when a `group` value has been assigned
									 * 
									 * @param {Object}
									 *            params the `NgTableParams`
									 *            requesting data
									...
									function getGroups(params) {
										...
										if (angular.isFunction(group)) {
											...
											// currently support for only one
											// group implemented
										...
										var adaptedFn = settings
												.getDataFnAdaptor(settings.getData);
										var gotData = $q.when(adaptedFn.call(
												settings, params));
										return gotData.then(function(data) {
											...
											if (sortDirection) {
												var orderByFn = ngTableDefaultGetData
														.getOrderByFn();
												var orderBy = convertSortToOrderBy(
														{
															value : sortDirection
														});
												result = orderByFn(result,
														orderBy);
											}
									...
									function getPath(obj, ks) {
										...
										// If we have reached an undefined
										// property
										// then stop executing and return
										// undefined
										if (obj === undefined)
											return void 0;

										// If the path array has no more
										// elements, we've reached
										// the intended property and return its
										// value
										if (ks.length === 0)
											return obj;

										// If we still have elements in the path
										// array and the current
										// value is null, stop executing and
										// return undefined
										if (obj === null)
											return void 0;
								 ...
								 */
								var settings = {
									// todo: remove $scope after acceptable
									// depreciation period as no longer required
									$scope : null, // set by ngTable controller
									$loading : false,
									dataset : null, // allows data to be set
													// when table is initialized
									total : 0,
									defaultSort : 'desc',
									filterOptions : angular
											.copy(defaultFilterOptions),
									groupOptions : angular
											.copy(defaultGroupOptions),
									counts : [10, 25, 50, 100],
									interceptors : [],
								...
								// run events during construction after the
								// initial create event. That way a consumer
								// can subscribe to all events for a table
		 ...
		 * @ngdoc service
		 * @name ngTableParams
		 * @description Backwards compatible shim for lowercase 'n' in
		 *              NgTableParams
		 */
 ...
 */
	...
	(function() {
		 ...
		 * 
		 * @description Each {@link ngTable ngTable} directive creates an
		 *              instance of `ngTableController`
		 */
		angular.module('ngTable').controller(
				'ngTableController',
				[
						'$scope',
						'NgTableParams',
						...
						'ngTableColumn',
						'ngTableEventsChannel',
						function($scope, NgTableParams, $timeout, $parse,
								$compile, $attrs, $element, ngTableColumn,
								ngTableEventsChannel) {
							var isFirstTimeLoad = true;
							$scope.$filterRow = {};
							$scope.$loading = false;

							// until such times as the directive uses an
							// isolated scope, we need to ensure that the check
							// for
							// the params field only consults the "own
							// properties" of the $scope. This is to avoid
							// seeing the params
							// field on a $scope higher up in the prototype
							// chain
							...
							// watch for when a new NgTableParams is bound to
							// the scope
							// CRITICAL: the watch must be for reference and NOT
							// value equality; this is because NgTableParams
							// maintains
							// the current data page as a field. Checking this
							// for value equality would be terrible for
							// performance
							// and potentially cause an error if the items in
							// that array has circular references
							...
							$scope.$watch('params.isDataReloadRequired()',
									onDataReloadStatusChange);

							this.compileDirectiveTemplates = function() {
								if (!$element.hasClass('ng-table')) {
									$scope.templates = {
										header : ($attrs.templateHeader
												? $attrs.templateHeader
												: 'ng-table/header.html'),
										pagination : ($attrs.templatePagination
												? $attrs.templatePagination
												: 'ng-table/pager.html')
									};
									$element.addClass('ng-table');
									var headerTemplate = null;

									// $element.find('> thead').length === 0
									// doesn't work on jqlite
							...
							this.loadFilterData = function($columns) {
								angular.forEach($columns, function($column) {
									...
									// if we're working with a deferred object
									// or a promise, let's wait for the promise
									/*
									 * WARNING: support for returning a $defer
									 * is depreciated
									 */
									if ((angular.isObject(result) && (angular
											.isObject(result.promise) || angular
										...
										delete $column.filterData;
										return pData.then(function(data) {
											// our deferred can eventually
											// return arrays, functions and
											// objects
											if (!angular.isArray(data)
													&& !angular
															.isFunction(data)
													&& !angular.isObject(data)) {
												// if none of the above was
												// found - we just want an empty
												// array
												data = [];
											}
											$column.data = data;
									...
									// otherwise, we just return what the user
									// gave us. It could be a function, array,
									// object, whatever
									else {
							...
							this.buildColumns = function(columns) {
								...
								(columns || []).forEach(function(col) {
											result.push(ngTableColumn
													.buildColumn(col, $scope,
															result));
										});
								return result
							};
							...
							this.setupBindingsToInternalScope = function(
									tableParamsExpr) {

								// note: this we're setting up watches to
								// simulate angular's isolated scope bindings

								// note: is REALLY important to watch for a
								// change to the ngTableParams *reference*
								// rather than
								// $watch for value equivalence. This is because
								// ngTableParams references the current page of
								// data as
								// a field and it's important not to watch this
								var tableParamsGetter = $parse(tableParamsExpr);
								$scope.$watch(tableParamsGetter, (function(
												params) {
							...
							function setupFilterRowBindingsToInternalScope() {
								if ($attrs.showFilter) {
									$scope.$parent.$watch($attrs.showFilter,
											function(value) {
												$scope.show_filter = value;
											});
								} else {
									$scope.$watch(hasVisibleFilterColumn,
											function(value) {
												$scope.show_filter = value;
											})
								}
							...
							function setupGroupRowBindingsToInternalScope() {
								...
								if ($attrs.showGroup) {
									...
									$scope.$parent.$watch(showGroupGetter,
											function(value) {
												$scope.$groupRow.show = value;
											});
									if (showGroupGetter.assign) {
										// setup two-way databinding thus
										// allowing ngTableGrowRow to assign to
										// the showGroup expression
										$scope.$watch('$groupRow.show',
												function(value) {
													showGroupGetter.assign(
															$scope.$parent,
															value);
												});
									}
								} else {
									$scope.$watch('params.hasGroup()',
											function(newValue) {
							...
							function getVisibleColumns() {
								return ($scope.$columns || []).filter(function(
												c) {
											return c.show($scope);
										});
							}
							...
							function commonInit() {
								ngTableEventsChannel.onAfterReloadData(
										bindDataToScope, $scope, isMyPublisher);
								ngTableEventsChannel
										.onPagesChanged(bindPagesToScope,
												$scope, isMyPublisher);
 ...
 */
	...
	(function() {
		 ...
		 *              {@link ngTableController ngTableController}.
		 */
					...
					return {
						...
						compile : function(element) {
							...
							angular.forEach(dataRow.find('td'), function(item) {
								...
								var parsedAttribute = function(attr) {
									...
									getter.assign = function($scope, value) {
										...
										if (parsedExpr.assign) {
											// we should be writing back to the
											// parent scope as this is where the
											// expression
											// came from
											parsedExpr.assign($scope.$parent,
													value);
										} else {
											localValue = value;
										}
								...
								var titleExpr = getAttrValue('title-alt')
										|| getAttrValue('title');
								if (titleExpr) {
								...
								// NOTE TO MAINTAINERS: if you add extra fields
								// to a $column be sure to extend ngTableColumn
								// with
								...
								columns.push({
									...
									show : el.attr("ng-if")
											? parsedAttribute('ng-if')
											: undefined
								});

								if (groupRow || el.attr("ng-if")) {
									// change ng-if to bind to our column
									// definition which we know will be writable
									// because this will potentially increase
									// the $watch count, only do so if we
									// already have an
									// ng-if or when we definitely need to
									// change visibility of the columns.
									// currently only ngTableGroupRow directive
									// needs to change visibility
									setAttrValue('ng-if', '$columns['
													+ (columns.length - 1)
													+ '].show(this)');
								}
							});
							return function(scope, element, attrs, controller) {
								scope.$columns = columns = controller
										.buildColumns(columns);

								controller
										.setupBindingsToInternalScope(attrs.ngTable);
 ...
 */
	...
	(function() {
		 ...
		 * @restrict A
		 * 
		 * @description A dynamic version of the {@link ngTable ngTable}
		 *              directive that accepts a dynamic list of columns
		...
		angular.module('ngTable').directive('ngTableDynamic', [function() {
			...
			return {
				...
				compile : function(tElement) {
					...
					return function(scope, element, attrs, controller) {
						var expr = controller
								.parseNgTableDynamicExpr(attrs.ngTableDynamic);

						controller
								.setupBindingsToInternalScope(expr.tableParams);
						controller.compileDirectiveTemplates();

						scope.$watchCollection(expr.columns,
								function(newCols/* , oldCols */) {
									scope.$columns = controller
											.buildColumns(newCols);
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').directive('ngTableColumnsBinding',
		 ...
		 * @description One-way data binds the $columns array generated by
		 *              ngTable/ngTableDynamic to the specified expression.
		 * 
		 * This allows the $columns array to be accessed outside of the html
		 * table markup
		 */
 ...
 */
	...
	(function() {
		 ...
		 * @restrict A
		 */
		angular.module('ngTable').directive(
				'ngTablePagination',
				['$compile', 'ngTableEventsChannel',
							...
							return {
								...
								link : function(scope, element/* , attrs */) {
									...
									scope.$watch('templateUrl', function(
										...
										var template = angular.element(document
												.createElement('div'));
										template.attr({
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').controller('ngTableFilterRowController',
		...
		function ngTableFilterRowController($scope, ngTableFilterConfig) {
			...
			$scope.getFilterPlaceholderValue = function(filterValue/*
																	 * ,
																	 * filterName
																	 */) {
				if (angular.isObject(filterValue)) {
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').directive('ngTableFilterRow',
				ngTableFilterRow);
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').controller('ngTableGroupRowController',
		...
		function ngTableGroupRowController($scope) {
			...
			function getGroupTitle(group) {
				return angular.isFunction(group) ? group.title : group
						.title($scope);
			}
			...
			function toggleDetail() {
				$scope.params.settings().groupOptions.isExpanded = !$scope.params
						.settings().groupOptions.isExpanded;
 ...
 */
	...
	(function() {
		...
		angular.module('ngTable').controller('ngTableSorterRowController',
		...
		function ngTableSorterRowController($scope) {
			...
			function sortBy($column, event) {
				...
				var sortingParams = (event.ctrlKey || event.metaKey)
 ...
 */
	...
	(function() {
		 ...
		 * @restrict A
		 * 
		 * @description Takes the array returned by $column.filterData and makes
		 *              it available as `$selectData` on the `$scope`.
		 * 
		 * The resulting `$selectData` array will contain an extra item that is
		 * suitable to represent the user "deselecting" an item from a `<select>`
		 * tag
		 * 
		 * This directive is is focused on providing a datasource to an
		 * `ngOptions` directive
		 */
		angular.module('ngTable').directive('ngTableSelectFilterDs',
		...
		function ngTableSelectFilterDs() {
			...
			// this is to allow this directive to be combined with other
			// directives that do
		...
		ngTableSelectFilterDsController.$inject = ['$scope', '$parse',
				'$attrs', '$q'];
		function ngTableSelectFilterDsController($scope, $parse, $attrs, $q) {
			...
			function getSelectListData($column) {
				var data = angular.isFunction($column.data)
						? $column.data()
						: $column.data;
				return $q.when(data);
			}
	...
	angular.module('ngTable').run(['$templateCache', function($templateCache) {
		$templateCache
				.put(
						...
						'<tr ng-show="show_filter" class="ng-table-filters"> <th data-title-text="{{$column.titleAlt(this) || $column.title(this)}}" ng-repeat="$column in $columns" ng-if="$column.show(this)" class="filter {{$column.class(this)}}" ng-class="params.settings().filterOptions.filterLayout===\'horizontal\' ? \'filter-horizontal\' : \'\'"> <div ng-repeat="(name, filter) in $column.filter(this)" ng-include="config.getTemplateUrl(filter)" class="filter-cell" ng-class="[getFilterCellCss($column.filter(this), params.settings().filterOptions.filterLayout), $last ? \'last\' : \'\']"> </div> </th> </tr> ');
		$templateCache
				.put(
						'ng-table/filters/number.html',
						'<input type="number" name="{{name}}" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="input-filter form-control" placeholder="{{getFilterPlaceholderValue(filter, name)}}"/> ');
		$templateCache
				.put(
						'ng-table/filters/select-multiple.html',
						'<select ng-options="data.id as data.title for data in $column.data" ng-disabled="$filterRow.disabled" multiple ng-multiple="true" ng-model="params.filter()[name]" class="filter filter-select-multiple form-control" name="{{name}}"> </select> ');
		$templateCache
				.put(
						'ng-table/filters/select.html',
						'<select ng-options="data.id as data.title for data in $selectData" ng-table-select-filter-ds="$column" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="filter filter-select form-control" name="{{name}}"> <option style="display:none" value=""></option> </select> ');
		$templateCache
				.put(
