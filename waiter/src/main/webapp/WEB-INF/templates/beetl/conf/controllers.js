	...
	.controller('BlankonCtrl', function($scope, $http, settings) {
		...
		$scope.supportIE = function() {
			...
			if (isIE8 || isIE9) { // ie8 & ie9
				...
				// placeholder-no-fix class will be skipped(e.g: we need this
				// for password fields)
		...
		$scope.soundPage = function() {
			...
			if ($('.page-sound').length) {
				ion.sound({
					...
					path : baseURL
							+ "assets/global/plugins/bower_components/ionsound/sounds/",
					preload : true
				});
		...
		// $http.get(settings.dataPath+'/navbarMessages') // Simple GET request
		// example :
		...
		// $http.get(settings.dataPath+'/navbarNotifications') // Simple GET
		// request example :
		// .success(function(data) {
		...
		// =========================================================================
		$scope.profile = [];
		// $http.get(settings.dataPath+'/profile') // Simple GET request example
		// :
		// .success(function(data) {
		// $scope.profile = data;
		// })
		...
		// =========================================================================
		$scope.chats = [];
		// $http.get(settings.dataPath+'/chat') // Simple GET request example :
		// .success(function(data) {
		// $scope.chats = data;
		// })
		// .error(function(data, status, headers, config) {
		...
		$scope.user = {};
		$http.get(settings.dataPath + '/user') // Simple GET request example :
				.success(function(data) {
							$scope.user = data;
						}).error(function(data, status, headers, config) {
						});
